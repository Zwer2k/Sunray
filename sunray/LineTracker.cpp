  ...
  if (!angleToTargetFits){
       ...
       ) 
    {
      ...
      if (gps.solution == SOL_FLOAT)        
        linear = min((DB64)setSpeed, 0.1); // reduce speed for float solution
      else
    ...
    if (!SMOOTH_CURVES) angular = max(-PI/16, min(PI/16, (DB64)angular)); // restrict steering angle for stanley
  }
